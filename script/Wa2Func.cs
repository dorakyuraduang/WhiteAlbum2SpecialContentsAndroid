...
public class Wa2Func
{
	...
	public bool M(List<Wa2Var> args)
	{
		...
		_engine.BgmInfo.Loop = args[2].Get()==1;
		_engine.BgmInfo.Volume = args[3].Get();
		_engine.BgmInfo.Idx = Array.IndexOf(Wa2Def.BgmSlot, _engine.BgmInfo.Id);
		// GD.Print("播放bgm");
		// GD.Print( args[1].Get());
	...
	public bool LoadBmp(List<Wa2Var> args)
	{
		...
		{
			texture.Texture = Wa2Resource.GetBmpImage(args[1].Get());
		}
		else
		{
			texture.Texture = Wa2Resource.GetBmpImage(args[1].Get());
